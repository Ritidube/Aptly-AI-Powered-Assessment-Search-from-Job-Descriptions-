...
def near_turn_cap(messages: List[Message], cap: int = 8, buffer: int = 1) -> bool:
    """True when we're close enough to the 8-turn cap that we must
    commit to a shortlist now instead of asking anything else.

    buffer=1 (previously 2): with turns_used() counting BOTH user and
    assistant messages, a buffer of 2 tripped this after only ~3 user
    exchanges (7 messages >= cap-2=6) — leaving very little room for
    genuine clarifying back-and-forth before being forced to recommend
    off whatever weak signal exists. buffer=1 still commits a full
    message before the hard 8-message cap (a real safety margin), just
    not so early that a 4th user turn is already too late to ask
    anything.

    NOTE: if the evaluator's "8-turn cap" actually means 8 *user*
    turns (not 8 total messages), turns_used() should count only
    user-role messages instead of len(messages) — that would roughly
    double the real budget. Confirm against the assignment spec before
    changing that, since it directly affects evaluator compliance.
    """
...
# How many of the most recent user turns feed into extract_requirements()
# by default in the routing/recommend decision path. Without a bound,
# a role/skill mentioned on turn 1 stays in the structured query
# forever — so a conversation that later pivots to an unrelated role
# (e.g. "backend engineer, python, sql" on turn 1, then "chef" /
# "fresher" several turns later) still has the old terms dominating
# retrieval, since to_query() weights structured terms 2x regardless
# of how long ago they were said. This doesn't attempt to detect a
# topic change explicitly (that's unreliable with keyword lists) — it
# just lets old signal decay after a few turns, which is enough to
# stop stale terms from outliving the topic they came from while still
# supporting normal incremental refinement over 2-3 turns.
REQUIREMENTS_RECENT_TURNS = 4
...
def extract_requirements(messages: List[Message], recent_turns: int = None) -> Requirements:
    """Build a structured requirements profile from user turns so far.
    Called fresh on every request since the API is stateless.

    recent_turns: if given, only the LAST `recent_turns` user messages
    are considered instead of the entire conversation. Callers that
    need the full running history for continuity (e.g. reconstructing
    a prior shortlist for refine/sticky-edit handling) should leave
    this unset. Callers deciding whether THIS turn is on-topic and
    recommend-worthy should pass REQUIREMENTS_RECENT_TURNS so old
    context can't keep forcing a stale recommendation indefinitely.
    """
    user_msgs = [m for m in messages if m.role == "user"]
    if recent_turns is not None:
        user_msgs = user_msgs[-recent_turns:]
