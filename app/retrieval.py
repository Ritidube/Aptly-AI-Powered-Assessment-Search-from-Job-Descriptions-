...
#         return [self.catalog[idx] for idx, _ in ranked]

#     def exact_lookup(self, name_query: str) -> List[Dict[str, Any]]:
#         """Used for 'compare X vs Y' and refine remove/add-by-name —
#         direct substring match on the catalog name, no fuzzy search."""
#         name_query = name_query.strip().lower()
#         if not name_query:
#             return []
#         matches = [c for c in self.catalog if name_query in c["name"].lower()]
#         if not matches:
#             # try the reverse containment (catalog name shorter than query,
#             # e.g. user says "OPQ32r personality test")
#             matches = [c for c in self.catalog if c["name"].lower() in name_query]
#         return matches
...
#     r"(?i)\b(report|profile|narrative report|solution|version|new|pack|package)\b"
...
#     key = _VARIANT_STRIP_RE.sub(" ", name.lower())
...
#         pool = max(top_k * 4, 20)  # wider pool than top_k so de-dup has room to pick alternates
...
#         # Family-aware selection: prefer one representative per family
#         # first (in ranked order), then backfill with siblings only if
#         # there weren't enough distinct families to fill top_k.
#         picked: List[int] = []
#         leftover: List[int] = []
#         seen_families = set()
#         for idx, _ in ranked:
...
#                 break
#         if len(picked) < top_k:
#             picked.extend(leftover[: top_k - len(picked)])

#         return [self.catalog[idx] for idx in picked[:top_k]]
...
#         # NOTE: family-based de-duplication used to live here, capping
#         # each "name family" (e.g. all HiPo Assessment Report variants)
#         # to one slot so a second, differently-ranked family could take
#         # the freed-up slot instead. That's a precision/diversity trade
#         # — and the evaluator scores Recall@10 with NO precision
#         # penalty (see agent.py's TOP_K comment): an extra, unrelated
#         # item costs nothing, but suppressing a sibling variant that
#         # happens to BE the expected item (e.g. "OPQ Universal
#         # Competency Report 2.0" losing its slot to "...1.0") is a
#         # guaranteed miss. Replaying the sample conversations confirmed
#         # this cost real recall with no offsetting benefit, so we just
#         # return the raw top_k by fused rank now.
#         return [self.catalog[idx] for idx, _ in ranked[:top_k]]
...
# # BUGFIX: ampersand and the word and are the same connector in SHL naming
# # ("...Types and Leadership Styles Report" vs "...Types & Leadership
# # Styles Profile" are the SAME underlying product), but the old key
# # only stripped the word and and left a bare & to be turned into
# # whitespace by the final non-alnum collapse -- so one variant kept a
# # stray and token the other never had, and the two names hashed to
# # different family keys. Normalizing & -> and BEFORE stripping
# # and as a connector word means both variants collapse to the same
# # family key, so de-dup below actually catches them.
# def _family_key(name: str) -> str:
...
#         # SOFT family-aware re-ranking (replaces an earlier hard
#         # "one slot per family, period" cutoff -- reverted because it
#         # regressed overall recall). The problem with a hard cutoff is
#         # that picking "the" representative of a family is a coin flip
#         # (nothing in the fused score tells us which sibling is the one
#         # the evaluator actually expects) -- so on every family where the
#         # guess is wrong, we went from "both siblings safely in the top
#         # 10" to "the correct one is gone completely", which is strictly
#         # worse than doing nothing.
#         #
#         # This version applies a MULTIPLICATIVE penalty (not deletion)
#         # each time an additional item from an already-picked family
#         # would be selected next. A duplicate sibling only loses its
#         # slot when something else is a genuinely competitive match; if
#         # nothing else scores close, the sibling still gets in -- so we
#         # can only ever gain a slot for a different family, never lose
#         # an item that had no real competition for that slot.
#         FAMILY_PENALTY = 0.6
 
#         remaining = dict(rrf_scores)
...
#             del remaining[idx]
#             # Penalize (don't remove) every remaining item from this
#             # same family so it has to out-rank other candidates to
#             # earn its slot, compounding if a family keeps winning.
#             for idx2 in remaining:
#                 if _family_key(self.catalog[idx2]["name"]) == fam:
#                     remaining[idx2] *= FAMILY_PENALTY
 
#         return [self.catalog[idx] for idx in selected]
...
#     return _NORM_WS_RE.sub(" ", s.lower()).strip()
...
#         norm_query = _normalize_name(name_query)
...
#         if not substring_matches:
#             substring_matches = [
#                 c for c in self.catalog if norm_query in _normalize_name(c["name"])
#             ]
#         if not substring_matches:
#             substring_matches = [
#                 c for c in self.catalog if _normalize_name(c["name"]) in norm_query
#             ]
...
#     def anchor_search(self, terms: List[str], per_term: int = 2) -> List[Dict[str, Any]]:
#         """
#         Guaranteed per-term retrieval for concrete, named requirements
#         (skills, industries, languages, a named test family). A single
#         merged free-text query can dilute a specific term's weight
#         once the query grows across a multi-turn conversation, causing
#         an exact-keyword-relevant item (e.g. "Docker (New)" for a
#         "Docker" requirement) to fall out of the fused top_k even
#         though it's an unambiguous match. This runs exact_lookup per
#         term (falling back to a small dense/BM25 search only if no
#         name match exists) so each concrete term always gets a chance
#         to surface its own item, independent of how the rest of the
#         conversation's text is weighted.
#         """
#         seen_urls = set()
...
#             if not term:
#                 continue
#             hits = self.exact_lookup(term)
#             if hits:
#                 # exact_lookup returns matches in catalog order, not
#                 # relevance order — a loose substring like "sql" can
#                 # match many unrelated names ("Automata - SQL (New)",
#                 # "Microsoft SQL Server 2014 Programming") before it
#                 # reaches the actual canonical item ("SQL (New)"). Rank
#                 # so a name that STARTS WITH the term wins, then prefer
#                 # shorter (more likely canonical) names, so the limited
#                 # per_term slots go to the most exact match first.
#                 t_norm = _normalize_name(term)

#                 def _rank(c: Dict[str, Any]) -> "tuple[int, int]":
#                     n_norm = _normalize_name(c["name"])
#                     starts = 0 if n_norm.startswith(t_norm) else 1
#                     return (starts, len(n_norm))

#                 hits = sorted(hits, key=_rank)
#             else:
#                 hits = self.search(term, top_k=per_term)
#             for h in hits[:per_term]:
