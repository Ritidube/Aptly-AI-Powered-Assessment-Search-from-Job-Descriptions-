...
def _apply_personality_boost(
    ...
    if not req.wants_personality_boost():
        return items
    # BUGFIX: this used to bail out whenever ANY "P" (Personality &
    # Behavior) category item was already present, on the theory that
    # personality was "already covered." That's wrong -- the catalog's
    # own P-category includes specialized OUTPUT REPORTS ("OPQ MQ
    # Sales Report", "OPQ Leadership Report") that are generated FROM
    # an OPQ32r administration, not substitutes for it. Verified
    # against the sample conversations: OPQ32r is expected ALONGSIDE
    # these specialized reports, not instead of them (e.g. a senior-
    # leadership battery wants OPQ32r + OPQ Universal Competency Report
    # + OPQ Leadership Report all at once). The precise "already
    # present" check belongs below, keyed on OPQ32r's own url/name,
    # not on the whole P category.
...
def resolve_refine_shortlist(
    ...
    # BUGFIX: _apply_personality_boost() was only ever called from
    # handle_chat()'s fresh "recommend" path. Every "refine" turn
    # (add/remove/confirm on an EXISTING shortlist) rebuilds the list
    # through THIS function instead and never called it -- so OPQ32r
    # could be present on turn 1 and then silently vanish on turn 2+
    # purely because this function had no personality-aware guarantee
    # of its own (verified: this is exactly what dropped OPQ32r from
    # several final-turn shortlists in the sample conversations even
    # though nothing ever asked to remove it). req below is used only
    # for that boost check, never for retrieval itself -- prefer the
    # full accumulated conversation signal, falling back to the prior
    # turn's if the latest turn alone added nothing new.
    boost_req = extract_requirements(messages)
    if boost_req.is_empty():
        boost_req = extract_requirements(messages[:-1])
    ...
    if action.full_replacement_terms:
        ...
        if replacement:
            boosted = _apply_personality_boost(
                replacement[:10], boost_req, retriever, exclude_urls=set()
            )
            return _dedup_by_url(boosted)[:10]
    ...
    if action.is_pure_confirmation and not include_items and not exclude_urls:
        if base_items:
            boosted = _apply_personality_boost(
                base_items, boost_req, retriever, exclude_urls=exclude_urls
            )
            return _dedup_by_url(boosted)[:10]
        full_req = extract_requirements(messages)
        fresh = retriever.search(full_req.to_query(), top_k=TOP_K) if not full_req.is_empty() else []
        boosted = _apply_personality_boost(fresh, boost_req, retriever, exclude_urls=exclude_urls)
        return _dedup_by_url(boosted)[:10]
    ...
    # Same personality guarantee the fresh "recommend" path already
    # gets, applied here for the first time -- reserves OPQ32r's slot
    # (evicting the lowest-ranked filler item, never a sticky/explicit
    # one) rather than hoping the family-de-dup ranking above happened
    # to keep it in.
    items = _apply_personality_boost(items, boost_req, retriever, exclude_urls=exclude_urls)
    return _dedup_by_url(items)[:10]
...
def handle_chat(
    ...
    # "recommend" or "refine"
    #
    # Bounded to recent turns: unlike resolve_refine_shortlist's own
    # full-history reconstruction (which needs the complete add/remove
    # trail to honor sticky edits), the query actually searched here
    # should reflect what the user is asking about NOW. Without the
    # window, a role/skill from turn 1 stays doubled-weighted in
    # to_query() forever, so a later topic switch (e.g. "chef",
    # several turns after an earlier "backend engineer, python, sql"
    # request) still gets swamped by the old terms.
    ...
    if items:
        ...
        logger.warning(
            "empty_shortlist",
            extra={"route_label": label, "query": req.to_query() if not req.is_empty() else ""},
        )
        reply = (
